...
// ── Build WHERE clause — scoped to current user only ────────
...
// ── Counts for stat cards (own logs only) ───────────────────
$total_stmt = $conn->prepare("SELECT COUNT(*) FROM audit_logs WHERE user_id = ?");
$total_stmt->bind_param("i", $current_user_id);
$total_stmt->execute();
$total_logs = (int) $total_stmt->get_result()->fetch_row()[0];
$total_stmt->close();

$today_stmt = $conn->prepare("SELECT COUNT(*) FROM audit_logs WHERE user_id = ? AND DATE(timestamp) = CURDATE()");
$today_stmt->bind_param("i", $current_user_id);
$today_stmt->execute();
$today_logs = (int) $today_stmt->get_result()->fetch_row()[0];
$today_stmt->close();

$create_stmt = $conn->prepare("SELECT COUNT(*) FROM audit_logs WHERE user_id = ? AND action_type = 'CREATE'");
$create_stmt->bind_param("i", $current_user_id);
$create_stmt->execute();
$create_count = (int) $create_stmt->get_result()->fetch_row()[0];
$create_stmt->close();

$update_stmt = $conn->prepare("SELECT COUNT(*) FROM audit_logs WHERE user_id = ? AND action_type = 'UPDATE'");
$update_stmt->bind_param("i", $current_user_id);
$update_stmt->execute();
$update_count = (int) $update_stmt->get_result()->fetch_row()[0];
$update_stmt->close();
...
<div class="page-hero fadeup">
    <div class="d-flex flex-wrap align-items-center justify-content-between mb-3 gap-3">
        <div>
            <div style="font-size:.72rem;text-transform:uppercase;letter-spacing:1px;color:rgba(255,255,255,.55);">HR Supervisor · Activity Monitor</div>
            <h4 class="text-white fw-bold mb-0 mt-1"><i class="fas fa-clipboard-list me-2" style="color:var(--primary-light);"></i>My Audit Trail</h4>
        </div>
        <div style="color:rgba(255,255,255,.6);font-size:.8rem;">
            <i class="fas fa-user-check me-1"></i> Your Activity Log &nbsp;·&nbsp; Latest 200 records
        </div>
    </div>
    <p class="text-white-50 small mb-0"><i class="fas fa-search me-1"></i>Review your own HRIS activity — approvals, endorsements, logins, and updates.</p>
</div>
...
<div class="row g-3 mb-4 fadeup-1">
    ...
    <div class="col-6 col-md-3">
        <div class="stat-card">
            <div class="stat-icon info"><i class="fas fa-edit"></i></div>
            <div class="stat-info">
                <h3><?php echo number_format($update_count); ?></h3>
                <p>Update Actions</p>
            </div>
        </div>
    </div>
...
<div class="content-card fadeup-2">
    <div class="card-header flex-wrap gap-2">
        <h5><i class="fas fa-clipboard-list me-2"></i>My Activity Logs</h5>
        <div class="d-flex align-items-center gap-2 flex-wrap ms-auto">

            <!-- Action Filter + Date Range -->
    ...
    <div class="card-body p-0">
            ...
            <table class="table table-hover" id="auditTable">
                ...
                <tbody>
                    <?php if ($audit_logs->num_rows === 0): ?>
                        <tr><td colspan="6" class="text-center text-muted py-4"><i class="fas fa-inbox me-2"></i>No activity found for the selected filters.</td></tr>
                    <?php else: ?>
        ...
        <div class="mobile-list-view d-block d-md-none p-3">
            <div class="student-list">
                <?php if ($audit_logs->num_rows === 0): ?>
                    <div class="text-center py-4 text-muted"><i class="fas fa-inbox me-2"></i>No activity found.</div>
                <?php else: ?>
                    <?php
