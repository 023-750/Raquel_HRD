...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'add_position' || $action === 'edit_position') {
        ...
        if ($action === 'add_position') {
            $stmt = $conn->prepare("INSERT INTO job_titles (job_title, department_id, rank_category_id, is_active) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("siii", $position_name, $department_id, $rank_category_id, $is_active);
            $stmt->execute();
        ...
        $stmt = $conn->prepare("UPDATE job_titles SET job_title = ?, department_id = ?, rank_category_id = ?, is_active = ? WHERE job_title_id = ?");
        $stmt->bind_param("siiii", $position_name, $department_id, $rank_category_id, $is_active, $position_id);
        $stmt->execute();
        $stmt->close();
...
<div class="page-hero fadeup">
    <div class="d-flex flex-wrap align-items-center justify-content-between mb-4 gap-3">
        <div>
            ...
            <h4 class="text-white fw-bold mb-0 mt-1"><i class="fas fa-briefcase me-2" style="color:#BD9414;"></i>Position Management</h4>
        </div>
    ...
    <div class="chart-card">
        <div class="cc-header">
            ...
            <div class="search-box">
                ...
                <input type="text" class="form-control form-control-sm" id="searchPosition" placeholder="Search positions...">
            </div>
        </div>
                        ...
                        <?php if ($positionCount === 0): ?>
                            <tr>
                                <td colspan="7" class="text-center text-muted py-4">
                                    ...
                                    No positions found. Add the missing positions here so they can be used in employee records.
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php while ($position = $positions->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td data-label="Department"><?php echo e($position['department_name'] ?: 'Unassigned'); ?></td>
                                    <td data-label="Rank"><?php echo e($position['rank_name'] ?: 'Unassigned'); ?></td>
                                    <td data-label="Employees"><span class="badge bg-info"><?php echo (int) $position['employee_count']; ?></span></td>
                                    ...
                                    <td data-label="Created"><small><?php echo formatDate($position['created_at']); ?></small></td>
                                    <td data-label="Actions">
                                        ...
                                        <button class="btn btn-sm btn-outline-danger"
                                            ...
                                            onclick="setDeletePositionTarget(
                                                <?php echo (int) $position['job_title_id']; ?>,
                                                '<?php echo e(addslashes($position['job_title'])); ?>',
            ...
            <form method="POST" action="">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="position_name" required placeholder="e.g. Regional Manager">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="rank_category_id">
                            ...
                            <?php foreach ($rankCategories as $rank): ?>
                                <option value="<?php echo (int) $rank['rank_category_id']; ?>"><?php echo e($rank['rank_name']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="is_active" id="addPositionActive" checked>
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-footer justify-content-center">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <a href="#" id="deletePositionConfirmBtn" class="btn btn-danger">Delete</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    function openEditPositionModal(id, name, departmentId, rankId, isActive) {
