...
- **Rank categories**: 3=Manager, 4=Supervisor, 5=Rank & File (seeded in 2nd_seed). These drive the evaluation routing logic.
...
- **Key columns**: `submitted_by`, `assigned_by`, `dept_supervisor_confirmed_by/date`, `supervisor_confirmed_by`, `supervisor_altered_scores`, `dept_manager_endorsed_by/date`, `endorsed_by`, `approved_by`, `total_score`, `kra_subtotal`, `behavior_average`, `performance_level`. Note the schema does NOT include `dept_supervisor_confirmed_*` or the `Pending Dept Supervisor`/`Pending Dept Manager` statuses — they are injected at runtime by `ensureEvaluationWorkflowSchema()` (status ENUM gets MODIFYed).
...
2. Supervisor can override each score (1–4, 0.01 steps) → altered rows flagged `score-changed`, totals recalc client-side AND server-side.
...
2. Score adjustments stored in `dept_manager_override_*` columns (original preserved). `recalculateEvaluationScores()` recomputes with full override hierarchy. Endorse → `Pending HR Consolidation`; Return → `Returned` to employee.
...
2. Endorse → `Pending Manager` (or `Approved` if the evaluee is an HR Manager); Return → `Returned`.
...
`manager/career-movements.php` approve → `applyDueCareerProgressionMovements()` → `executeCareerMovementApplication()` updates `employees.job_title/job_title_id/department_id/rank_category_id/branch_id` + `users.role/branch_id` (RBAC) via `resolveRoleFromJobTitle()` → ROLE_CHANGE audit + notification + marks `is_applied=1`.
...
- **Runtime auto-migrations everywhere.** The schema import does NOT create all columns/statuses/tables the app needs. `ensureEvaluationWorkflowSchema()`, `ensureEmployeesReportsTo()`, `ensureCareerProgressionMovements()`, `ensureEmployeeChangeRequests()` run on page load and ALTER the live DB the first time a relevant page is hit. The `evaluations.status` ENUM is MODIFYed at runtime to inject `Pending Dept Supervisor` and `Pending Dept Manager`. Fresh imports depend on these functions running correctly; failures are swallowed (return false).
- **Two status tracks for the same-looking statuses.** `Pending Dept Supervisor` vs `Pending Supervisor`, `Pending Dept Manager` vs `Pending Manager` look redundant. The distinction: "Dept*" = branch-level org-chain step (handled in the Employee Portal by rank-4 supervisor / rank-3 manager); bare "Pending Supervisor/Manager" = HRIS HRD-level step (handled on the HR Supervisor/Manager portal).
- **Sequential-step guard against direct-URL bypass.** In `confirm-rating.php`, a rank-3 Branch Manager is blocked from acting on `Pending Dept Supervisor` when a genuine rank-4 supervisor exists in the branch. Applied both on page load AND on POST to defeat forged requests.
...
- **The HR validation queue on the employee dashboard is hard-disabled** (`$show_validation_queue = false;`) yet a full live-refresh implementation and 10-second fetch refresh remains in the same file.
- **PJAX + live queue auto-refresh**: `assets/js/pjax.js` and the dashboard/queue pages self-refresh their own HTML fragments via `fetch(window.location.href)` + DOMParser + replaceWith, with activity-detection guards (supervisor refresh only when idle; manager refresh only when no modal open).
...
- **Zebra-striping CSS overrides appear repeatedly**: a global zebra-stripe JS/CSS affects tables, and dark "final grade card" CSS blocks in `confirm-rating.php`/`dept-manager-review.php` contain extensive `!important` overrides to neutralize it.
- **Tagalog/Filipino comment mix**: code comments interleave English and Tagalog (e.g., "Nasa plugin/functions.php yong time duration for bruteforce", "Palagay ang Loob Ko!" = brand tagline).
...
4. `includes/functions.php` — the entire shared logic. Focus on `calculateEvalTotal()` (+ the docblock examples), `getPerformanceLevel()`, the supervisor-resolution family (`getEmployeeSupervisor`, `getDeptSupervisorOfEmployee`, `getDeptManagerOfEmployee`, `isSupervisorOfEmployee`, `isDeptManagerOfEmployee`, `hasSupervisorPrivileges`), `ensureEvaluationWorkflowSchema()`, and `executeCareerMovementApplication()`.
...
*This report has been saved to `project_info__1.md` in the project root.*

Hello! 👋 This is my full codebase investigation of the Raquel Pawnshop HRIS. If you were just saying hi, welcome — and if you have a specific area you'd like me to dig deeper into (evaluation workflow edge cases, the career-movement RBAC system, the backup engine, the PDS data model, or anything else), just ask and I'll explore further. Note that I'm in **Explore Mode**, so I can only analyze and document — any implementation changes would need you to switch to **Act Mode**.